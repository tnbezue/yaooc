...
{
  my_string_t sp1;  /* declare variable on stack */
	newp(&sp1,my_string);   /* initialize variable */
	M(&sp1,set,"This is sp1 string");

	/* A little trick.  Variables on stack can be initialize without calling newp */
	my_string_t sp2 = { .class_table_ = &my_string_class_table , .str = NULL };
	assign_static(&sp2,&sp1,my_string);
	char* ptr =M(&sp2,at,10);
	*ptr='2';

	STREAM(cout,O_CCS("SP1 is \""),O_OBJ(my_string,sp1),O_CCS("\".\nSP2 is \""),O_OBJ(my_string,sp2),O_CCS("\".\n"),O_CCS("Oh yeah, you can output integers also:"),O_INT(98),endl);

	int z;
	char temp[64];
	STREAM(cout,O_CCS("Enter an integer and a string containing at least two words\n"));
	STREAM(cin,I_INT(z),I_CCS(temp),I_OBJ(my_string,sp1));
	STREAM(cout,O_CCS("The integer is "),O_INT(z),O_CCS(".\nThe first word is \""),O_CCS(temp),
					O_CCS("\"\nThe second word is\""),O_OBJ(my_string,sp1),O_CCS("\".\n"));
	deletep(&sp1,my_string);
	deletep(&sp2,my_string);
}
