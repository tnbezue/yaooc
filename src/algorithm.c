...
/*
  Caller has to update size of container if needed
  new size is (iterator returned)-first;
*/
      ...
      const_iterator sf,const_iterator sl)
{
  ...
  } else {  // all members are equal
    pos=last - (s_last-s_first);
    if(pos < first) // search range > source range
      pos=last;
  }
  return pos;
}
      ...
      const_iterator sf,const_iterator sl)
{
  ...
  yaooc_private_const_iterator s_last=sl;
	size_t type_size=yaooc_sizeof(ti);
  if(s_first != s_last) { // Check for zero length range
