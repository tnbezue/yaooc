...
{
	return yaooc_container_erase(p,pos);
}

iterator yaooc_ordered_set_erase_range(pointer p,iterator first,iterator last)
{
	return yaooc_container_erase_range(p,first,last);
}
iterator yaooc_ordered_set_find(pointer p,const_iterator value)
{
	iterator end=yaooc_container_end(p);
	yaooc_ordered_container_find_result_t fr=yaooc_ordered_container_find_private(p,value);
	if(!fr.found_)
		return end;
//	return yaooc_ordered_container_begin(p)+fr.index_;
  return fr.position_;
}

const_iterator yaooc_ordered_set_cfind(const_pointer p,const_iterator value)
{
	const_iterator end=yaooc_container_end((pointer)p);
	yaooc_ordered_container_find_result_t fr=yaooc_ordered_container_find_private((pointer)p,value);
	if(!fr.found_)
		return end;
//	return yaooc_ordered_container_cbegin(p)+fr.index_;
  return fr.position_;
}
...
yaooc_ordered_set_pointer yaooc_ordered_set_sunion_(pointer p,const_pointer o)
{
	...
	const char* is=(const char*)yaooc_container_cbegin(o);
	const char* end=(const char*)yaooc_container_cend(o);
