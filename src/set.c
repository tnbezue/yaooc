...
iterator yaooc_set_find(pointer p,const_iterator value)
{
	...
	if(!fr.found_)
		return end;
	return yaooc_index_container_begin(p)+fr.index_;
}
/*
const_iterator yaooc_set_cfind(const_pointer p,const_iterator value)
{
	const_iterator end=yaooc_container_cend(p);
	yaooc_index_container_find_result_t fr=yaooc_index_container_private_find((pointer)p,value);
	if(!fr.found_)
		return end;
	return yaooc_index_container_cbegin(p)+fr.index_;
